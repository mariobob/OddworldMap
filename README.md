...
```bash
python3 tools/build_map.py --game AO --disc "/path/to/Oddworld - Abe's Oddysee.bin"
python3 tools/build_map.py --game AO --levels R2,R6   # subset while iterating
```

(`--disc` can be omitted if `$ODDWORLD_DISC_AO` points at the image.)
