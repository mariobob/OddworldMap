...
- **hover** any object for decoded details — door destinations (level/path/door#), switch IDs, path-transition targets, continue-point zones, Mudokon state (Oddysee job / Exoddus work state + mood), and enemy start state (a Slig's patrol/sleep/listen, whether a Slog starts asleep and the switch that angers it)
