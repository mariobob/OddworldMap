...
- with **"List a screen's objects on click"** enabled in Settings, clicking/tapping anywhere else on a screen lists everything on it, grouped by category — hover a row to outline that object on the map, click it to jump there. On touch devices, where there is no hover, this panel is how you inspect a screen
